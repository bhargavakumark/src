...
int Usage()
{
    printf("<prog> : <devname> <read-offset> <compute-offset>\n");
}
...
int main(int argc, char *argv[])
{
    ...
    uint64_t computeoffset = strtoull(argv[3], NULL, 10);
    printf("dev = %s offset = %lu computeoffset = %lu\n", device.c_str(), offset, computeoffset);
    ...
    memcpy(buf+114688-32, &computeoffset, sizeof(computeoffset));
#if 0
    buf[114688-32+0] = 0;
    buf[114688-32+1] = 64;
    buf[114688-32+2] = 0;
    ...
    printf("0 = %x 1 = %x 30 = %x 114688-32-1 = %x 114688-32-3 = %x\n", buf[0], buf[1], buf[30], buf[114688-32-1], buf[114688-32-3]);

    int64_t cksum[4];
    zio_checksum_SHA256(buf, 114688, cksum);

    char *outfile = "sha256.out";
#if 0
